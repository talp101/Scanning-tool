...
class Scanner():
    ...
    def tcp_scan(self):
        p = IP(dst=self.dst_ip, src=self.src_ip) / TCP(sport=self.src_port, dport=self.dst_ports[0], flags="S")
        ...
        if str(type(resp)) == "<type 'NoneType'>":
            ...
            if resp.getlayer(TCP).flags == 0x12:
                send_rst = sr(
                    IP(dst=self.dst_ip, src=self.src_ip) / TCP(sport=self.src_port, dport=self.dst_ports[0], flags="AR"),
                    timeout=10)
